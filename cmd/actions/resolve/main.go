...
func main() {
	...
	flag.StringVar(&fallback, "fallback", "", "fallback repository if --repo is empty")
	flag.Parse()

	repo = strings.TrimSpace(repo)
	if repo == "" {
		repo = strings.TrimSpace(fallback)
	}
	if repo == "" {
		log.Fatal("unable to determine repository that hosts release assets")
	}
